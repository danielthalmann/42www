...
{{--                <th class="p-4 border border-slate-600">end_at          </th> --}}
                    ...
                    <th class="p-4 border border-slate-600">has_coalition   </th>
                    <th class="p-4 border border-slate-600">user42_id       </th>
                    <th class="p-4 border border-slate-600 text-right">pool_month_year</th>
                    <th class="p-4 border border-slate-600">correction_point  </th>
                ...
                <tr>
                    <td class="    border border-slate-600 w-12" dtat-id="{{$user->user_id}}"><img src="{{ $user->image_url }}" alt=""></td>
                    <td class="p-4 border border-slate-600">{{ $user->user_id           }}</td>
                    <td class="p-4 border border-slate-600">{{ $user->name              }}</td>
                    <td class="p-4 border border-slate-600">{{ $user->login             }}</td>
...
{{--                <td class="p-4 border border-slate-600">{{ $user->end_at ? $user->end_at->format('d.m.Y') : null           }}</td> --}}
                    ...
                    <td class="p-4 border border-slate-600">{{ $user->has_coalition     }}</td>
                    <td class="p-4 border border-slate-600">{{ $user->user42_id         }}</td>
